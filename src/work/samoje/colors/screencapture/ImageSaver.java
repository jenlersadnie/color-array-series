...
public class ImageSaver {
    public static void saveImageForGrid(final Color[][] grid, final Filter filter, final File file)
    {
        final BufferedImage bufferedImage = new BufferedImage(grid[0].length, grid.length,
    ...
    public static void sectionWrite(final Color[][] grid, final Filter filter,
            final int firstRow, final int lastRow, final BufferedImage bufferedImage)
    {
        final Thread[] threads = new Thread[lastRow-firstRow];
        for (int x = firstRow; x < lastRow; x++) {
            threads[x-firstRow] = new Thread(new WriteRow(x, grid[x], filter, bufferedImage));
    ...
    public static class WriteRow implements Runnable
    {
        ...
        public WriteRow(final int xIndex, final Color[] rowValues, final Filter filter, final BufferedImage image)
        {
            this.xIndex = xIndex;
