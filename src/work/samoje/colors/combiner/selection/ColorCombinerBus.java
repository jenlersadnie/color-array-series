...
public class ColorCombinerBus extends Observable implements CombinerProvider {
    ...
    public void update(final EnumSet<CombineMethod> combineMethods, final int multiplier)
    {
        if (combineMethods.isEmpty()) {
            this.combineMethods = EnumSet.allOf(CombineMethod.class);
        } else {
    ...
    public ColorCombiner getCombiner() {
        final List<ColorCombiner> chosenCombiners = combiners(multiplier).entrySet().stream()
    ...
    public Map<CombineMethod, ColorCombiner> combiners(final int value)
    {
        ...
        combineMap.put(CombineMethod.RGB_SUM_MOD, new RGBSumMod(value, MAX_MULTIPLIER));
        combineMap.put(CombineMethod.RGB_SCALE_AVG, new RGBScaleAvg(value, MAX_MULTIPLIER));
        combineMap.put(CombineMethod.NOISY_AVG, new RGBNoisyAvg(value, MAX_MULTIPLIER));
